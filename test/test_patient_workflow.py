...
def test_patient_creation():
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            logger.info(
                f"⏰ Expires in: {response_data.get('expires_in_days', 'Unknown')} days"
            )
            ...
            if verification_code:
                ...
                verify_result = test_verification(
                    base_url, verification_token, verification_code
                )
...
def test_duplicate_patient():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if response_data.get("status") == "info":
                logger.info(
                    "✅ Correct behavior: Forgot password email sent for existing account"
                )
                logger.info(f"📝 Message: {response_data.get('message')}")
                return True
        else:
...
if __name__ == "__main__":
    ...
    logger.info(
        f"🏥 Patient Creation: {'✅ Success' if patient_result else '❌ Failed'}"
    )
    logger.info(
        f"🔄 Duplicate Handling: {'✅ Success' if duplicate_result else '❌ Failed'}"
    )
    ...
    if patient_result and duplicate_result:
        ...
        if not email_valid:
            logger.warning(
                "⚠️ Note: Email configuration needs attention for production use"
            )
        exit(0)
    else:
        logger.error("\n💥 Some tests failed!")
