...
def test_duplicate_user():
    ...
    test_user = {"email": "duplicatetest@example.com", "password": "TestPassword123!"}
...
def test_verification_link(token):
    ...
    try:
        ...
        response = requests.get(
            f"{base_url}/auth/verify-link/{token}", verify=False, timeout=10
        )
...
def test_server_running():
    ...
    try:
        ...
        response = requests.get(
            "https://localhost:20001/auth/verify", verify=False, timeout=5
        )
        if response.status_code in [
            405,
            410,
        ]:  # Method not allowed or gone - means server is running
