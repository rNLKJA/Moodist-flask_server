...
def test_clinician_creation():
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            logger.error(
                f"Clinician creation failed with status code: {response.status_code}"
            )
            return None
...
def test_clinician_password_reset():
    ...
    reset_request_data = {"email": "dr.test@unimelb.edu.au"}
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.info(
                "In production, you would complete the reset using the code from email"
            )
...
def test_generic_doctor_creation():
    ...
    logger.info(
        "Testing generic doctor account creation (should also work for clinicians)..."
    )
...
def main():
    ...
    logger.info(
        f"Clinician Login (unverified): {'EXPECTED FAIL' if not login_result else 'UNEXPECTED PASS'}"
    )
