...
os.environ["SECRET_KEY"] = (
    "Univeristy-of-Melbourne-Psychiatry-Department-Moodist-Application"
)
...
def test_duplicate_user_workflow():
    ...
    test_user = {"email": "testduplicate@example.com", "password": "TestPassword123!"}
    ...
    try:
        ...
        if response1.status_code == 201:
            ...
            verify_response = requests.get(
                f"{base_url}/auth/verify-link/{token}", verify=False, timeout=30
            )
            ...
            if verify_response.status_code == 200:
                ...
                logger.info(
                    f"✅ User verified successfully with ID: {verify_data.get('user_id')}"
                )
                ...
                if response2.status_code == 200:
                    ...
                    if data2.get("status") is False:
                        logger.info(
                            "✅ Correctly returned status: false for duplicate user"
                        )
                        logger.info(f"✅ Message: {data2.get('message')}")
                        logger.info(
                            f"✅ Redirect to reset: {data2.get('redirect_to_reset')}"
                        )
                        return True
                    else:
                        logger.error(
                            f"❌ Expected status: false but got: {data2.get('status')}"
                        )
                        return False
                else:
                    logger.error(
                        f"❌ Expected status 200 for duplicate user but got: {response2.status_code}"
                    )
                    return False
