...
def main():
    ...
    ssl_context = None
    if (
        app.config.get("SSL_ENABLED", False)
        and app.config.get("SSL_CERT")
        and app.config.get("SSL_KEY")
    ):
    ...
    print(f"Network accessible: {'Yes' if host == '0.0.0.0' else 'Localhost only'}")

    # Run the application
    app.run(host=host, port=port, ssl_context=ssl_context, debug=app.config["DEBUG"])
