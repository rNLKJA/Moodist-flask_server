...
class Config:
    ...
    SESSION_TYPE = (
        "filesystem"  # Options: filesystem, redis, memcached, mongodb, sqlalchemy
    )
    ...
    SESSION_COOKIE_SECURE = (
        SSL_ENABLED  # Only send cookies over HTTPS when SSL is enabled
    )
...
class ProductionConfig(Config):
    ...
    if os.path.exists(os.path.join(os.getcwd(), "certs/global-cert.pem")):
        SSL_CERT = os.path.join(
            os.getcwd(), get_env("SSL_CERT", "certs/global-cert.pem")
        )
        SSL_KEY = os.path.join(os.getcwd(), get_env("SSL_KEY", "certs/global-key.pem"))
    else:
        SSL_CERT = os.path.join(
            os.getcwd(), get_env("SSL_CERT", "certs/trusted-cert.pem")
        )
        SSL_KEY = os.path.join(os.getcwd(), get_env("SSL_KEY", "certs/trusted-key.pem"))
