...
def check_today_log():
    ...
    try:
        ...
        return jsonify({"success": True, "has_logged_today": has_logged})
...
def save_mood_scores():
    ...
    try:
        ...
        if not data or "scores" not in data:
            return (
                jsonify({"success": False, "error": "Missing scores in request"}),
                400,
            )
...
def get_user_logs():
    ...
    try:
        ...
        if user_type == "patient":
            ...
            return jsonify({"success": True, "logs": simplified_logs})
...
def get_recent_days_logs():
    ...
    try:
        ...
        if user_type == "patient":
            ...
            return jsonify({"success": True, "days": days, "logs": simplified_data})
        elif user_type in ["doctor", "admin"]:
            # Table format for clinicians and admins
            table_data = {
            ...
            # Default format for unknown user types
            return jsonify(
                {"success": True, "days": days, "logs_by_date": logs_by_date}
            )
...
def get_patient_logs(patient_id):
    ...
    try:
        ...
        summary = {
            ...
            "average_score": (
                sum(log.get("total_score", 0) for log in logs) / len(logs)
                if logs
                else 0
            ),
        }
