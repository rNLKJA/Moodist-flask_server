...
def health():
    ...
    response_data = {
        ...
        "ssl": {"enabled": ssl_enabled, "protocol": "https" if ssl_enabled else "http"},
        "access": {"type": access_type, "external_url": external_url},
    ...
    return jsonify(
        {
            ...
            "description": current_app.config.get(
                "APP_DESCRIPTION", "A mood tracking application"
            ),
            "creator": "rNLKJA",
            ...
            "mobile_development": {
                ...
                "instructions": (
                    "Use HTTP URLs for Expo/React Native apps"
                    if not ssl_enabled
                    else "Consider disabling SSL for mobile development"
                ),
            },
            "cors": {
                "enabled": True,
