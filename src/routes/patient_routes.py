...
def remove_patient_connections(client, patient_id, old_patient_unique_id):
    ...
    try:
        ...
        connections = client.find_documents(
            "connections", {"patient_id": patient_id, "status": "active"}
        )
        ...
        for connection in connections:
            ...
            logger.info(
                f"Deleted connection: {connection['connection_id']} for patient {old_patient_unique_id}"
            )

        logger.info(
            f"Removed {len(connections)} connections for patient {old_patient_unique_id}"
        )
...
def generate_new_unique_id():
    ...
    try:
        ...
        patients = client.find_documents(
            "patient", {"unique_id": current_unique_id}, limit=1
        )
        ...
        logger.info(
            f"Patient {patient_id} changed unique_id from {current_unique_id} to {new_unique_id}"
        )
...
def change_unique_id():
    ...
    try:
        ...
        if not validate_unique_id(current_unique_id) or not validate_unique_id(
            new_unique_id
        ):
            return (
                jsonify(
        ...
        existing_patients = client.find_documents(
            "patient", {"unique_id": new_unique_id}, limit=1
        )
...
def get_patient_info():
    ...
    try:
        ...
        patients = client.find_documents("patient", {"unique_id": unique_id}, limit=1)
