...
def connection_status():
    ...
    if connected:
        return jsonify(
            {"status": "connected", "url": couch.couch_url, "database": couch.db_name}
        )
    else:
        return (
            jsonify(
...
def create_message():
    ...
    if result:
        ...
        return jsonify({"error": "Failed to create message", "status": "error"}), 500
...
def get_messages():
    ...
    return jsonify({"messages": messages, "count": len(messages), "status": "success"})
