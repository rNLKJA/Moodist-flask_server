...
def require_jwt_auth(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        try:
            ...
            tokens = client.find_documents(
                "auth_tokens", {"access_token": token, "active": True}, limit=1
            )
...
def connect_to_patient():
    ...
    try:
        ...
        patients = client.find_documents(
            "patient", {"unique_id": patient_unique_id}, limit=1
        )
        ...
        existing_connections = client.find_documents(
            "connections",
            {
        ...
        logger.info(
            f"Connection created: {request.clinician_id} -> {patient_unique_id}"
        )
...
def list_connections():
    ...
    try:
        ...
        connections = client.find_documents(
            "connections", {"clinician_id": request.clinician_id, "status": "active"}
        )
...
def remove_connection():
    ...
    try:
        ...
        logger.info(
            f"Connection removed: {request.clinician_id} -> {connection.get('patient_unique_id')}"
        )

        return (
            jsonify({"success": True, "message": "Connection removed successfully"}),
            200,
        )
...
def get_patient_mood_logs(patient_unique_id):
    ...
    try:
        ...
        connection_exists = client.find_documents(
            "connections",
            {
                "clinician_id": request.clinician_id,
                "patient_unique_id": patient_unique_id,
        ...
        mood_logs = client.find_documents(
            "mood_logs", {"user_id": patient_id, "type": "mood_log"}
        )
        ...
        logger.info(
            f"Retrieved {len(formatted_logs)} mood logs for patient {patient_unique_id} by clinician {request.clinician_id}"
        )
...
def get_patient_reference_lines(patient_unique_id):
    ...
    try:
        ...
        logger.info(
            f"Retrieved {len(reference_lines)} reference lines for patient {patient_unique_id} by clinician {request.clinician_id}"
        )
...
def add_patient_reference_line(patient_unique_id):
    ...
    try:
        ...
        logger.info(
            f"Added reference line {ref_id} for patient {patient_unique_id} by clinician {request.clinician_id}"
        )
...
@connection_bp.route(
    "/patient/<patient_unique_id>/reference-lines/<int:ref_id>", methods=["PUT"]
)
@require_jwt_auth
def update_patient_reference_line(patient_unique_id, ref_id):
    ...
    try:
        ...
        logger.info(
            f"Updated reference line {ref_id} for patient {patient_unique_id} by clinician {request.clinician_id}"
        )
...
@connection_bp.route(
    "/patient/<patient_unique_id>/reference-lines/<int:ref_id>", methods=["DELETE"]
)
@require_jwt_auth
def delete_patient_reference_line(patient_unique_id, ref_id):
    ...
    try:
        ...
        reference_lines = [
            ref for ref in reference_lines if ref.get("ref_id") != ref_id
        ]
        ...
        logger.info(
            f"Deleted reference line {ref_id} for patient {patient_unique_id} by clinician {request.clinician_id}"
        )

        return (
            jsonify(
                {"success": True, "message": "Reference line deleted successfully"}
            ),
            200,
        )
