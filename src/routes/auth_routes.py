...
from src.utils.token_generator import (
    ...
    generate_verification_link_token,
    hash_password,
    verify_link_token,
    verify_password,
)
...
def get_database_name(user_type):
    ...
    database_mapping = {"patient": "patient", "doctor": "clinician", "admin": "moodist"}
        ...
        for db in databases_to_check:
            try:
                ...
                try:
                    ...
                    if doc:
                        logger.info(
                            f"ID {unique_id} already exists as _id in {db} database"
                        )
                        id_exists = True
                        break
                except Exception as e:
                ...
                if result:
                    logger.info(
                        f"ID {unique_id} already exists as unique_id in {db} database"
                    )
                    id_exists = True
                    break
...
def login():
    ...
    try:
        ...
        user_type = data.get(
            "user_type"
        )  # Optional, will search all DBs if not provided
...
def logout():
    ...
    try:
        ...
        return jsonify({"status": True, "message": "Logout successful"}), 200
...
def check_session():
    ...
    try:
        if current_user.is_authenticated:
            ...
            return jsonify({"status": True, "authenticated": False}), 200
...
def create_user(user_type):
    ...
    try:
        ...
        if existing_user:
            if existing_user.get("status") in [
                "verified",
                "active",
            ] or existing_user.get("is_verified"):
                ...
                logger.info(
                    f"Resending verification link for existing unverified user: {email}"
                )
        else:
            # Create new user
        ...
        verification_token = generate_verification_link_token(
            email, user_type, expires_in_days=7
        )
        ...
        if not password_hash:
            return (
                jsonify({"status": "error", "message": "Failed to hash password"}),
                500,
            )
        ...
        if existing_user:
            ...
            if not unique_id:
                ...
                return (
                    jsonify(
                        {"status": "error", "message": "Failed to generate unique ID"}
                    ),
                    500,
                )
        ...
        verification_url = (
            f"https://445983cd85dc.ngrok-free.app/auth/verify-link/{verification_token}"
        )
...
def verify_link(token):
    ...
    try:
        ...
        if not email or not user_type:
            return jsonify({"status": "error", "message": "Invalid token payload"}), 400
        ...
        if stored_token != token:
            return (
                jsonify({"status": "error", "message": "Verification token mismatch"}),
                400,
            )
        ...
        if not unique_id:
            ...
            unique_id = generate_unique_id(user_type, client)
            logger.info(
                f"Generated new unique_id {unique_id} for user {email} during verification"
            )

        if not unique_id:
            return (
                jsonify(
                    {"status": "error", "message": "Failed to generate unique user ID"}
                ),
                500,
            )
        ...
        try:
            ...
            return (
                jsonify(
                    {"status": "error", "message": "Failed to complete verification"}
                ),
                500,
            )
...
def change_password():
    ...
    try:
        ...
        if not new_password_hash:
            return (
                jsonify({"status": "error", "message": "Failed to hash new password"}),
                500,
            )
        ...
        try:
            ...
            return (
                jsonify({"status": "error", "message": "Failed to save new password"}),
                500,
            )
...
def resend_verification():
    ...
    try:
        ...
        # Check if user is already verified
        if user_doc.get("status") in ["verified", "active"] or user_doc.get(
            "is_verified", False
        ):
            return (
                jsonify(
...
def request_password_reset():
    ...
    try:
        ...
        if not user_doc:
            ...
            return (
                jsonify(
                    {
                        "status": True,
                        "message": "If your email exists, a code has been sent.",
                    }
                ),
                200,
            )
        ...
        try:
            ...
            return (
                jsonify({"status": False, "message": "Failed to process request"}),
                500,
            )
...
def reset_password():
    ...
    try:
        ...
        try:
            ...
            return (
                jsonify({"status": False, "message": "Invalid expiration format"}),
                400,
            )
        ...
        try:
            ...
            return (
                jsonify({"status": False, "message": "Failed to update password"}),
                500,
            )
...
"""
        ...
        return (
            jsonify({"status": True, "message": "Support message sent successfully"}),
            200,
        )
...
def request_email_change():
    ...
    try:
        ...
        try:
            ...
            if not user_data:
                logger.error(
                    f"Could not find user document for ID {user_id} in database {db_name}"
                )
                return (
                    jsonify(
                        {
        ...
        logger.info(
            f"User data retrieved successfully. Current email: {user_data.get('email')}"
        )
        ...
        logger.info(
            f"Generated verification code for email change. Expires at: {expires_at.isoformat()}"
        )
        ...
        try:
            client.save_document(db_name, user_data)
            logger.info(
                f"Email change requested for user {user_id} to new email {new_email}"
            )
        except Exception as e:
...
def verify_email_change():
    ...
    try:
        # Log the request for debugging
        logger.info(
            f"Email change verification request received for user ID: {current_user.id}"
        )
        ...
        logger.info(
            f"User data retrieved successfully. Email: {user_data.get('email')}"
        )
                ...
                try:
                    ...
                    if datetime.utcnow() < lockout_until_dt:
                        remaining_seconds = int(
                            (lockout_until_dt - datetime.utcnow()).total_seconds()
                        )
                        logger.warning(
                            f"User {user_id} is locked out for {remaining_seconds} more seconds"
                        )
                        return (
                            jsonify(
                                {
        ...
        if not verify_password(stored_hash, password):
            ...
            logger.warning(
                f"Password verification failed for user {user_id} during email change"
            )
                ...
                jsonify(
                    {
                        ...
                        "attempts_remaining": max(
                            0, 5 - user_data["email_change_failed_attempts"]
                        ),
                    }
                ),
                401,
        ...
        logger.info(
            f"Checking verification code. Stored code exists: {stored_code is not None}, "
        ...
        if not stored_code or not expires_at or not new_email:
            missing = []
            if not stored_code:
                missing.append("email_change_code")
            if not expires_at:
                missing.append("email_change_expires_at")
            if not new_email:
                missing.append("new_email")
            logger.error(
                f"Missing email change data: {', '.join(missing)}. Did you request an email change first?"
            )
            return (
                jsonify(
                    {
        ...
        if code != stored_code:
            # Log the error for debugging
            logger.warning(
                f"Invalid verification code for user {user_id} during email change. Expected: {stored_code}, Got: {code}"
            )
        ...
        if datetime.utcnow() > expires_at_dt:
            logger.warning(
                f"Verification code expired. Expired at: {expires_at}, Current time: {datetime.utcnow().isoformat()}"
            )
            return (
                jsonify(
                    {
        ...
        logger.info(
            f"Updating email from {old_email} to {new_email} for user {user_id}"
        )
        ...
        try:
            client.save_document(db_name, user_data)
            logger.info(
                f"Successfully changed email for user {user_id} from {old_email} to {new_email}"
            )
        except Exception as e:
...
def change_user_id():
    ...
    try:
        ...
        if new_user_id == old_unique_id:
            logger.warning(
                f"Generated ID {new_user_id} is the same as current ID {old_unique_id}, trying again"
            )
            new_user_id = generate_unique_id(current_user.user_type, client)
        ...
        try:
            client.save_document(db_name, user_data)
            logger.info(
                f"User {user_id} changed unique_id from {old_unique_id} to {new_user_id}"
            )
        except Exception as e:
        ...
        try:
            ...
            logger.info(
                f"Would sever all connections for user with old ID: {old_unique_id}"
            )
    ...
    try:
        ...
        if existing_user:
            if existing_user.get("status") in [
                ...
                logger.info(
                    f"Clinician {email} already verified, returning status: false"
                )
                return (
                    jsonify(
                        {
                ...
                logger.info(
                    f"Resending verification link for existing unverified clinician: {email}"
                )
        else:
        ...
        # Generate verification link token
        verification_token = generate_verification_link_token(
            email, "doctor", expires_in_days=7
        )
        if not verification_token:
            return (
                jsonify(
        ...
        try:
            ...
            return (
                jsonify(
                    {"status": "error", "message": "Failed to create clinician account"}
                ),
                500,
            )
        ...
        verification_url = (
            f"https://{domain_name}/auth/verify-link/{verification_token}"
        )

        # Send clinician-specific verification email
        email_subject = (
            "Verify Your Moodist Clinician Account - Action Required (7 Days)"
        )
        email_template = f"""
        <html>
            <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
...
"""
        ...
        if not email_sent:
            return (
                jsonify({"status": False, "message": "Failed to send reset email"}),
                500,
            )
...
def clinician_reset_password():
    ...
    try:
        ...
        if not user_doc:
            return (
                jsonify(
                    {"status": False, "message": "Invalid email or verification code"}
                ),
                400,
            )
        ...
        if not stored_code or not expires_at:
            return (
                jsonify({"status": False, "message": "No active reset code found"}),
                400,
            )
        ...
        if code != stored_code or datetime.utcnow() > expires_at_dt:
            return (
                jsonify(
                    {"status": False, "message": "Invalid or expired verification code"}
                ),
                400,
            )
        ...
        if not password_hash:
            return (
                jsonify({"status": False, "message": "Failed to hash new password"}),
                500,
            )
...
"""
        ...
        )  # Send confirmation but don't fail if it doesn't send
