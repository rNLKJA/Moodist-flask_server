...
class User(UserMixin):
        ...
        try:
            ...
            for db_name in databases:
                try:
                    ...
                    if "not_found" not in str(e) and "illegal_database_name" not in str(
                        e
                    ):
                        logger.warning(
                            f"Error checking for user in {db_name}: {str(e)}"
                        )
                    continue
    ...
    def authenticate(cls, email, password, user_type=None):
        ...
        if not user.is_verified:
            logger.warning(
                f"Authentication partial: User {email} password correct but not verified"
            )
