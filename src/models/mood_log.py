...
class MoodLog:
    ...
    def has_logged_today(cls, user_id):
        ...
        try:
            ...
            selector = {"user_id": user_id, "log_date": today_date}
    ...
    def save_mood_log(cls, user_id, scores):
        ...
        try:
            ...
            for q in range(1, 6):
                ...
                if (
                    not isinstance(scores[q_key], int)
                    or scores[q_key] < 0
                    or scores[q_key] > 3
                ):
                    raise ValueError(
                        f"Score for question {q} must be an integer between 0 and 3"
                    )
            ...
            if result:
                logger.info(
                    f"Saved mood log for user {user_id} on {today_date} with total score {total_score}"
                )
                return {
                    "success": True,
                    "id": result.get("id"),
            ...
            logger.error(
                f"Validation error saving mood log for user {user_id}: {str(e)}"
            )
    ...
    def get_user_logs(cls, user_id, limit=None):
        ...
        try:
            client = CouchDBClient()
            selector = {"user_id": user_id, "type": "mood_log"}
    ...
    def get_recent_days_logs(cls, user_id, days=7):
        ...
        try:
            ...
            filtered_logs = [
                log for log in all_logs if log.get("log_date") in date_range
            ]
    ...
    def get_patient_logs(cls, patient_id, date_range=None, limit=None):
        ...
        try:
            client = CouchDBClient()
            selector = {"user_id": patient_id, "type": "mood_log"}
            ...
            if date_range and isinstance(date_range, dict):
                ...
                if start_date and end_date:
                    ...
                    # Sort logs by log_date in descending order (most recent first)
                    filtered_logs.sort(
                        key=lambda x: x.get("log_date", ""), reverse=True
                    )
