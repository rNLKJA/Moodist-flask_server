...
def hash_password(password):
    ...
    if not pepper:
        logger.warning(
            "Password pepper not configured. Using less secure password hashing."
        )
...
def verify_link_token(token, max_age_seconds=None):
    ...
    try:
        ...
        logger.info(
            f"Successfully verified link token for email: {payload.get('email')}"
        )
        return payload
...
def verify_password_token(token, password, max_age_seconds=None):
    ...
    try:
        ...
        logger.info(
            f"Successfully verified password token for user type: {payload.get('user_type')}"
        )
        return payload
